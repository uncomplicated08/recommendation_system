...
from models.recommender import load_model, get_recommendations
import pandas as pd
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.df = pd.read_csv('data/dataset.csv')  # Adjust path as necessary
        self.df['user_id'] = pd.factorize(self.df['CustomerID'])[0]
    ...
    def display_recommendations(self):
        ...
        recommendations = get_recommendations(self.model, self.df, user_id)
        self.text_edit.clear()
        self.text_edit.append("Recommendations for user {}: \n".format(user_id))
        for idx in recommendations:
            product = self.df.loc[self.df['product_id'] == idx, 'Description'].values[0]
